...
import { HkTableAddButton, HkTableRemoveButton, HkSaveButton, HkCancelButton, HkDropdownButton } from 'components/HkButton.jsx';
...
class InvoiceForm extends React.Component {
        ...
        this.payColumns = [
            {
                ...
                width: 100,
                render: (value, record) => {
                    return format.number(value, this.localCurrency);
                },
            }, {
    ...
    componentWillUpdate() {
        if (this.state.currentLanguage !== store.getState().language.current) {
            this.createColumns();
        }
        this.calculateTableSize();
    }
        ...
        return (
            <div>
                <Row justify='end' type='flex' >
                    ...
                    <Col span={4} className='table-footer-sum-col'>{format.number(this.state.model.total || 0, this.localCurrency)}</Col>
                    <Col span={3} />
                ...
                {this.state.model.taxes && this.state.model.taxes.map((tax, i) => {
                    return <Row justify='end' type='flex' key={i}>
                        ...
                        <Col span={4} className='table-footer-sum-col'>{format.number(tax.value, this.localCurrency)}</Col>
                        <Col span={3} />
                    </Row>;
                })}
                <Row justify='end' type='flex' >
                    <Col span={4} className='table-footer-sum-col'>{localeInv.generaltotal}</Col>
                    <Col span={4} className='table-footer-sum-col'>{format.number(this.state.model.generaltotal || 0, this.localCurrency)}</Col>
                    <Col span={3} />
                </Row>
                <Row justify='end' type='flex' >
                    <Col span={4} className='table-footer-sum-col'>{localeInv.remain}</Col>
                    <Col span={4} className='table-footer-sum-col' style={remainStyle}>{format.number(this.state.model.remain||0, this.localCurrency)}</Col>
                    <Col span={3} />
                </Row>
            </div>
        );
        ...
        return (
            <div>
                <Row justify='end' type='flex' >
                    ...
                    <Col span={4} className='table-footer-sum-col'>{format.number(this.state.model.totalPayment, this.localCurrency)}</Col>
                    <Col span={3} />
                </Row>
                <Row justify='end' type='flex' >
                    <Col span={4} className='table-footer-sum-col'>{localeInv.remain}</Col>
                    <Col span={4} className='table-footer-sum-col' style={remainStyle}>{format.number(this.state.model.remain, this.localCurrency)}</Col>
                    <Col span={3} />
                </Row>
            </div>
        );
    ...
    render() {
        const localeInv = locale.getCategory('invoice');
        const pageTitle = locale.getPage(this.state.invoice_type.toLowerCase() + '_invoice_form');
            ...
            <div ref={(invForm) => { this.invForm = invForm }}>
                <HkTitle title={pageTitle}>
                    <Row type="flex" justify="end">
                        <Col><HkSaveButton onClick={this.onSave} /></Col>
                        <Col><HkDropdownButton dropdownMenu={this.state.headerOptionsMenu} /></Col>
                        <Col><HkCancelButton onClick={this.onCancel} /></Col>
                    </Row>
                </HkTitle>
